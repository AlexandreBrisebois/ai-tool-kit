...
4. **Secret Management**: Ensure `GOOGLE_API_KEY` is present in the environment or `.env` (managed by the tool).

## 🛠️ Tool Interaction
- **Path**: `./generator.py`
- **Primary Command**: `python ./generator.py --json "[prompt]" --output "[path]"`
- **Custom Critique**: `python ./generator.py --critique "[criteria_or_file]" ...`
...
2. Run `python ./generator.py --json "Futuristic city terminal icon, isometric, neon accents" --output assets/icons/terminal.webp`.
